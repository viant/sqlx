...
func TestNewValidation(t *testing.T) {
	...
	var testCases = []testCase{
		...
		{
			description: "09 fk validation passed with not has flag",
			driver:      "sqlite3",
			dsn:         "/tmp/sqllite.db",
			initSQL: []string{
				"CREATE TABLE IF NOT EXISTS dept01 (id INTEGER PRIMARY KEY, name TEXT, desc TEXT, unk TEXT)",
		...
		{
			description: "11 not null validation passed with not has flag",
			driver:      "sqlite3",
			dsn:         "/tmp/sqllite.db",
			initSQL:     []string{},
			data: &Record{
		...
		{
			description: "13 unique validation passed with not has flag",
			driver:      "sqlite3",
			dsn:         "/tmp/sqllite.db",
			initSQL: []string{
				"CREATE TABLE IF NOT EXISTS v03 (id INTEGER PRIMARY KEY, name TEXT, dept_id INTEGER, desc TEXT, unk TEXT)",
...
//	for i, testCase := range testCases[9:10] {
//		fmt.Printf("#CASE: %d/%d - %s\n", i+1, len(testCases), testCase.description)
//
//		db, err := sql.Open(testCase.driver, testCase.dsn)
//		if !assert.Nil(t, err, testCase.description) {
