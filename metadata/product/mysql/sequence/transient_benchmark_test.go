...
// Benchmark comparing Handle vs HandleLegacy using the same DB setup.
func BenchmarkTransientHandleVsLegacy(b *testing.B) {
	dsn := os.Getenv("MYSQL_BENCH_DSN")
	if dsn == "" {
		b.Skip("set MYSQL_BENCH_DSN to run this benchmark")
	}
	...
	catalog := ""
	table := "t_seq_bench_cmp"
	idCol := "id"

	// Default to 1 record; override with env if needed
	records := int64(1)
	if v := os.Getenv("MYSQL_BENCH_RECORDS"); v != "" {
		if n, err := strconv.ParseInt(v, 10, 64); err == nil && n > 0 {
			records = n
	...
	b.Run("HandleLegacy", func(b *testing.B) {
		b.ReportAllocs()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			var seq sink.Sequence
	...
	b.Run("Handle N-rec", func(b *testing.B) {
		b.ReportAllocs()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			var seq sink.Sequence
	...
	b.Run("HandleLegacy N-rec", func(b *testing.B) {
		b.ReportAllocs()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			var seq sink.Sequence
