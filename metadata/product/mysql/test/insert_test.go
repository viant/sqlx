...
func TestService_Exec_Mysql(t *testing.T) {
	...
	var useCases = []struct {
		...
		},
		{
			...
			affected: 4,
			// Historically, a zero ID in the first record enabled presetting for the
			// entire input and overwrote the explicit IDs below, producing IDs 1-4.
			// Since 9bdbefb, the sequencer presets only zero-valued IDs and preserves
			// explicit IDs (91-93), so the last inserted ID is 93.
			lastID: 93,
			options: []option.Option{
				option.BatchSize(3),
				dialect.PresetIDWithTransientTransaction,
